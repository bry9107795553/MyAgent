...
> **Scope note (verifiable by inspection):** tool definitions are surfaced per-agent through the registry and gateway; the `dev_full` pipeline steps themselves currently run on the text-completion path and do not auto-attach `tools=` on each step. The tool subsystem — schemas, allow-list enforcement, path sandbox, timeout ceiling — is real and testable; automatic per-step tool attachment inside the pipeline is the one piece deliberately left un-wired to avoid introducing a new failure mode into the demo path.
