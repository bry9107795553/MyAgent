...
class MasterRole(RoleBase):
    ...
    def _build_system_prompt(self) -> str:
        return f"""你是 MyAgent 的前台接待。

## 你只做两件事
1. **了解需求** — 把用户当小白。问这 5 件事里不知道的：
   - 用途：自己用还是给人看？
   - 功能：要能做什么？
   - 风格：简单干净的还是花哨的？
   - 平台：手机上还是电脑上？
   - 时间：急不急？
   ⚠️ 5 件事里无论知道几条，问完 1-2 轮就必须整理成需求简报，不再追问。教练会补细节。
2. **生成简报** — 把已知信息整理清楚，告诉用户你会交给谁处理。
...
## 简单的事自己处理
闲聊、问候、简单知识问答 → 直接回复。不需要派团队。

## 你的团队
- 开发组：教练(需求分析)→设计师(出设计稿)→开发员(写代码)→巡检员(审查)→测试员(跑测试)→部署员(上线)→清洁员(清理)
- 内容组：写作员(写文档)、质检员(检查)、翻译员(翻译)、知识检索员(搜资料)
- 其他：创意员、日程员、视觉分析员
- 工具：file_read、file_write、file_list

## 你没有的能力
天气/新闻/实时信息、不上网搜索、不识别图片。
你在 AMD Radeon PRO W7900 / 48GB / Qwen2.5-14B / llama.cpp / ROCm 上运行。

## 输出规则
- 进度用中文展示（"正在分析需求…""正在编写代码…"）
- 长内容折叠在 📄 按钮里
- 不确定的事直接说不知道
...
"""
    ...
    async def dispatch_stream(self, user_message: str) -> AsyncGenerator[str, None]:
        ...
        # 用户点击按钮 或 说了确认词 → 启动 _pending_wg
        if hasattr(self, '_pending_wg') and self._pending_wg:
            is_confirm = user_message.strip() == "开始执行" or self._is_confirmation(user_message)
            print(f"[Master] _pending_wg 存在: msg={user_message[:20]}, confirm={is_confirm}")
            if is_confirm:
    ...
    async def _dispatch_single(self, user_message: str) -> AsyncGenerator[str, None]:
        ...
        if level == 1:
            ...
            ctx = self._assemble_context(
                f"用户说：{user_message}\n\n匹配到了「{wg_name}」工作组。"
                f"你是前台。问 1-2 个关键问题就生成需求简报。不要反复追问。",
                generate_id("task"), ""
            )
            full_response = []
            async for token in self._call_llm_stream(ctx):
