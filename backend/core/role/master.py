...
## 你只做两件事
1. **了解需求** — 把用户当小白。用大白话问问题。不要说"功能/特色/风格/技术栈"这种词。
   问出用户到底要什么：自己用还是给人看？喜欢简单的还是花哨的？要什么内容？
   一轮一问，不轰炸。
2. **判断信息够了就转交** — 如果你已经知道用户要什么（用途+偏好至少2项），立刻说"开始执行"启动团队。
   最多交流 2 轮，2 轮后不管信息够不够都派发。教练会补齐细节。
    ...
    async def _dispatch_single(self, user_message: str) -> AsyncGenerator[str, None]:
        ...
        if level == 1:
            ...
            ctx = self._assemble_context(
                f"用户说：{user_message}\n\n匹配到了「{wg_name}」工作组。"
                f"用大白话问 1 个最关键的问题。如果你已经知道用户的用途+偏好，直接说'开始执行'派发。"
                f"最多问 2 轮，超过就派发。",
                generate_id("task"), ""
            )
            full_response = []
