...
# 测试 12: 断网后皮肤生成仍应成功（验证辅助功能同样是本地推理）
# 说明：P0 合规清理后本项目已无任何云端 API 通道，皮肤生成同样走本机
#       llama.cpp。断网时它应当照常工作 —— 这是"全本地"的正面证据。
print_info "测试 12: 断网后皮肤生成应仍然成功（全本地推理）..."
LOCAL_SKIN=$(curl -s --connect-timeout 30 -X POST http://localhost/api/skins/generate \
    -H "Content-Type: application/json" \
    -d '{"prompt": "生成一个赛博朋克风格皮肤"}' 2>&1)

if echo "$LOCAL_SKIN" | grep -q "skin_id\|colors\|success"; then
    print_pass "断网下皮肤生成成功 —— 辅助功能亦为本地推理，无远程依赖"
else
    print_fail "断网下皮肤生成失败（请确认 llama-server 已启动）"
fi
...
if [ $FAIL -eq 0 ]; then
    ...
    echo "  - 断网环境下，皮肤生成等辅助功能同样正常 (无任何远程 API 依赖)"
    echo "  - 恢复网络后，所有功能恢复正常"
    exit 0
else
    echo -e "${RED}========================================${NC}"
